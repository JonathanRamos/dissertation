...
\S~\ref{cha:math-prel}.\ref{sec:finite-markov-chain}. The {\em expected commute time}
...
and graphs sparsification \cite{spielmand08:_graph}. Expected commute
time is also the limiting behaviour for the families of
...
Proposition~\ref{prop:18}, show that $\Delta_{\delta}$ is EDM-2.
\begin{proposition}
  \label{prop:10}
...
\begin{proof}
  ...
  Definition~\ref{def:4}). By Ger\u{s}gorin circle theorem, the
...
the sum of one over the degree of the vertices. These observations means that
...
$G$ for any time scale $t$ only keeps tracks of paths of even length
...
\|$. This can be seen as follow.  Let $p$ be the spectral radius of
...
\mathbf{Q})^{-1}\bm{\Pi}^{-1})$. We also recalled from
