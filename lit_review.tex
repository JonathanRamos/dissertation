...
representations of data, namely feature vectors, proximity matrices,
...
%
%
\noindent An intuitive way of representing data is as a matrix where
each row is associated with an object and each column is associated
with a feature. The entries in row $i$ then comprise a vector of
...
$\Omega$ a subset of $\mathbb{R}^{p}$. We assume
in this work that the feature vector representation of data is
...
\times \Omega$ satisfying the following conditions
\begin{enumerate}[(i)]
...
distances on $\Omega$ include the $p$-norm distance (Minkowski
...
therefore appear superfluous. However, there are many application
domains where it is much easier to give a rough estimate of the
...
dimensionality. The space consumption $O(n^2)$ of the proximity
...
risk and a control group. The computation of the proximities in their
study is very computational intensive. It's therefore possible that
it's not desirable or feasible to compute the full proximity matrix
for a larger group of hippocampus images. The above reasons indicate
that the proximity representation is a valid and useful representation
in its own right.
\section{Dimension reduction algorithms}
\label{sec:dimens-reduct-algor}
This section surveys some of the existing literature on dimension 
reduction and manifold learning algorithms. 
Dimension reduction, to be intentionally vague, is the reduction
...
\citep{torgersen52:_multid,gower66:_some}. These techniques are viewed
...
are grouped under the umbrella of \emph{nonlinear} dimension reduction. For
example, Sammon's mapping \citep{j.69:_nonlin} is an early example of
a nonlinear dimension reduction technique. \\ \\
%
\noindent
Some of the early approaches to nonlinear dimension reduction was
through the extension of PCA\@. In the 1980's, nonlinear extension of
PCA, namely principal curves and surfaces, was proposed by
\citet{hastie84:_princ} and \citet{hastie89:_princ}. Meanwhile,
extensions of PCA using neural networks were also done in the neural
networks community, e.g., \citet{rubner89,oja92:_princ}.  See also the
survey article of \citet{oja02:_unsup} for more information on work in
this area. Recently, other extensions of PCA were also proposed, such
as regularized principal manifolds \citep{smola01:_regul_princ_manif} ,
probabilistic principal component analysis \citep{tipping99:_mixtur}
...
and micro-array data \citep{wouters03:_graph}. In image data, the
...
image lie in a space of dimension $10^4$. However, in a collection
...
camera. In micro-array DNA data, the number of observed genes for a
...
notion of connectivity on $G$. This lead to interest in understanding
the spectral properties of the combinatorial graph Laplacian. See for
example the book of \citet{cvetkovic80:_spect_graph_theor_applic} and
the survey articles of \citet{merris94:_laplac} and
\citet{mohar91:_graph}. The normalized Laplacian $\bm{\mathcal{L}}(G)$
rose in popularity through the work by Chung and her
coauthors. The monograph \citet{chung97:_spect_graph_theor} contains
an overview of the normalized Laplacian and its connection with graph
theory, harmonic analysis, and other diverse disciplines. Laplacian
matrices of graphs had also appeared in the context of spectral
partitioning and clustering, e.g.,
\citet{scott90:_featur,shi00:_normal,ng02,weiss9:_segmen}. Laplacian
matrices also appeared in manifold learning, where it was observed
that they share connections with the Laplace-Beltrami operator on
Riemannian manifolds. This lead to the study of convergence of
discrete Laplacians
...
1905, in an issue of
Nature, for help in finding references or solutions to the following
problem
\begin{quote}
\begin{verbatim}
A man starts from a point O and walks l yards in a straight line;
he then turns through any angle whatever and walks another l yards 
in a second straight line. He repeats this process n times. I require
the probability that after these n stretches he is at a distance between
r and r + dr from his starting point, O.
\end{verbatim}
\end{quote}
After hearing from Lord Rayleigh that his problem is the same as that
of the composition of $n$ iso-periodic vibrations of unit amplitude
...
point''\footnote{\url{http://www.nature.com/physics/looking-back/pearson/index.html}}.
...
random walks on graphs have been in widespread use in machine
...
that the eigenvectors of various matrix representations of data points,
...
of manifold learning algorithms embed data points by using eigenvalues
and eigenvectors of some matrix representation of the data. With the
transition to very large data sets, a lot of work has been done in
...
%
\noindent
The structure for this section is as follows. We discuss the classic
dimension reduction techniques, namely PCA and classical MDS in
\S~\ref{sec:princ-comp-analys} and \S~\ref{sec:classical-mds}.  We
then review several well known manifold learning algorithms, namely
Isomap, LLE, Hessian eigenmaps, Laplacian eigenmaps, and diffusion
maps, in \S~\ref{sec:isomap} through \S~\ref{sec:diffusion-maps}. Some
of the techniques, such as Isomap, Laplacian eigenmaps
\citep{belkin03:_laplac}, and diffusion maps
...
preserves the neighborhood structure of the data points. This is done
...
loss function. The tangent spaces represent the neighborhood
structure at each point and their alignment gives a global coordinate
system to embed the data points. For additional surveys of manifold
learning and dimension reduction algorithms, including some that are
...
reduction techniques are either extensions of PCA or employ PCA in
the construction of their embeddings and it is therefore important that
we are acquainted with the ideas behind PCA\@. This subsection
summarizes
...
\item Compute the covariance matrix $\mathbf{S} =
  \frac{1}{n}\tilde{\mathbf{X}}^{T}\tilde{\mathbf{X}}$\footnote{The
    ...
    $1/(n-1)$, in which case the resulting matrix might be referred to
...
Classical multidimensional scaling \citep{torgersen52:_multid,gower66:_some}
...
\item Compute $\mathbf{B}(\Delta) = \tau(\Delta^{(2)})$ where the
  $\tau$ transform is as defined in
  \S~\ref{sec:distance-geometry}. Note that if
  $\Delta$ is EDM-1, then $\mathbf{B}(\Delta)$ is positive
  semidefinite.
...
\begin{equation}
 ...
 \|_F^{2} 
\end{equation}
subject to the rank constraint $\mathrm{rank}(\mathbf{Z}) \leq
...
introduced as a framework for nonlinear dimension reduction. One
...
\item Construct an $m$-dimensional embedding of $\mathcal{X}$ by
...
based on the assumption that, under appropriate sampling of
$\mathcal{X}$ and appropriate constructions of the graph $G$, the
shortest path distance between $x_i$ and $x_j$ on $G$ converges to the
geodesic distance between $x_i$ and $x_j$ on $\mathcal{M}$, as the
number of sampled points increases to
...
distances using Landmark MDS\@. As has been observed by
...
i.e., they do not learn a mapping function from the domain of the data
...
the data points that are not in the sample. Out-of-sample extensions
for Isomap, LLE, MDS, and Laplacian eigenmaps exist and can be found
in \citet{bengio04:_out_lle_isomap_mds_eigen,trosset08}.
%
%
...
neighborhood preserving embeddings through linear reconstructions of a
point from its neighbors. Let $\mathcal{X} = \{x_1,x_2, \dots, x_N\}$
...
\item Choose a parameter $k$ representing the size of the
  neighborhoods of the data points. For each $x_i \in \mathcal{X}$,
  find its $K$ nearest neighbors and denote the resulting set of data
...
that is composed of the $d$ eigenvectors corresponding to the
...
Hessian eigenmap \citep{donoho03:_hesian} has been described as a locally
linear embedding technique for high-dimensional data,
c.f. \citep{roweis00:_nonlin} and
\S~\ref{sec:locally-line-embedd}. Hessian eigenmaps are motivated by
the assumption that the data points lie on a $d$-dimensional
...
procedure proceeds as follows \citep{donoho03:_hesian}
\begin{enumerate}
\item Identify neighbors. For each $m_i$, set $\mathcal{N}(m_i)$ to be
  the set of the $K$ nearest neighbors of $m_i$. For each
  neighborhood $\mathcal{N}(m_i)$, set $\mathbf{M}^{(i)}$ to be the
...
\end{array} \right ]}^{T}
  \end{equation}
  where $u_1, u_2, \dots, u_d$ denotes an orthonormal basis for the
  ...
  \end{gather}
  The truncated polynomial in Eq.~\eqref{eq:85} can be fit using
  ...
  have orthonormal columns, then $\mathbf{H}^{(i)}$ coincides with the
...
technique for high dimensional data that embeds using a system of
...
\item Assign a similarity measure $\omega \colon E \mapsto
  \mathbb{R}^{\geq 0}$. Once again, there are various 
...
\item Let $\mathbf{L}$ be the combinatorial Laplacian of $G$ with
  ...
  \end{equation}
\item Let $\lambda_1 \leq \lambda_2 \leq \dots \geq \lambda_N$ be the
...
$\mathcal{M}$. Roughly speaking, if one constructs a complete graph $G$
...
Laplacian matrix $L$ converge to the corresponding eigenvalues and
eigenfunctions of the Laplace-Beltrami operator $\Delta$ on
$\mathcal{M}$ \citep{belkin08:_towar_theor_found_laplac,belkin06:_conver_laplac_eigen}. \\ \\
%
%
...
the above optimization problem can be found as follows
 ...
 of $\mathbf{D}^{-1} \mathbf{L}$ coincide with the eigenvalues of
 ...
 eigenmaps are therefore the solution of the optimization problem in
 Eq.~\eqref{eq:89}.
%
%
\subsection{Diffusion maps}
\label{sec:diffusion-maps}
Diffusion maps \citep{coifman06:_diffus_maps} are an embedding
...
diffusion maps proceed as follows (c.f. Isomap \S~\ref{sec:isomap}
...
\item Let $\lambda_1 \geq \lambda_2 \geq \dots \geq \lambda_N$ be the
 ...
 where $\mathbf{f}_1$ is constant and thus ignored.
\end{enumerate}
The construction of the graph $G$ is an important part of diffusion
maps. \citet{coifman06:_diffus_maps} argues for the following approach
to constructing $G$. Let $\mathcal{M}$ be a smooth, compact
submanifold of $\mathbb{R}^{n}$ and suppose that the distribution of
points on $\mathcal{M}$ is given by a density function $q$. Let $h$ be
a radial function. Fix $\alpha \in \mathbb{R}$. The family of
anisotropic diffusion kernels is given by the following procedure
\begin{enumerate}
...
different choices of $\alpha$. Special cases include $\alpha = 0$,
...
\item Represent the data as a weighted, or possibly unweighted,
  ...
  eigenmaps and diffusion maps weigh the edges by the Gaussian
  kernel with some bandwidth parameter.
\item Compute new proximities between the vertices. Isomap computes
  shortest path distance, diffusion maps compute diffusion
  distances. A variation of Laplacian eigenmaps computes expected
  commute time.
\item Embed the graph in a Euclidean space using the new
...
namely $\epsilon$-ball and $K$ nearest neighbors. There exist results
that indicate that different graph construction leads to vastly
...
steps. The latter chapters of this work are concerned with the
interplay between the second and third steps of the above recipe.  We
want to mention two bodies of works that are slightly related to the
view of dimension reduction algorithms expounded here. \citet{ham04}
showed that several dimension reduction algorithms such as Isomap and
Laplacian eigenmaps can be viewed as instances of Kernel
...
\label{sec:contr-this-work}
The contributions of this work are as follows. We present a novel
framework for the construction of Euclidean distances on undirected
graphs in Chapter~\ref{cha:dist-undir-graphs}. The framework employs
the concept of random walks on graphs. Expected commute times and
diffusion distances are examples of distance measures that can be
obtained under this framework. We also investigate the question of how
to embed the resulting Euclidean distances. Two competing approaches
are presented in Chapter~\ref{cha:graph-metr-dimens}, namely the
embedding through classical MDS and the embedding through the
eigensystem of the transition matrix. We show that several well-known
manifold learning algorithms, namely, Laplacian eigenmaps and
diffusion maps, can be interpreted as algorithms that construct
low-dimensional representation of data by embedding the appropriate
distance measures. This interpretation is novel in the decoupling of
the distance measures from the embeddings. Furthermore, the decoupling
of the distance measures from the embeddings allows one to investigate
the properties of these algorithms from two different perspectives. As
an example, we present in Chapter~\ref{cha:graph-metr-dimens} an
anomaly that could arise in the embeddings constructed by diffusion
maps. This anomaly suggest itself when we look at the notion of
diffusion distances associated with diffusion maps. \\ \\
%
\noindent We then investigate in Chapter~\ref{cha:dist-direct-graphs}
the question of a framework for the construction of Euclidean
distances on directed graphs. Except for some scattered results in the
literature showing that a particular distance measure, e.g., expected
commute times, on directed graphs is a Euclidean distance measure,
there has not been a systematic investigation of distance measures on
directed graphs. We present two results summarizing this
investigation. The first result indicates that such a framework, if it
exists, would be much more restrictive than its counterpart for
undirected graphs. The second result is a framework for constructing
Euclidean distance measures on directed graphs that is dependent of
the graphs. This is in contrast to the framework for
undirected graphs which is independent of the graphs. \\ \\
%
\noindent We also consider the problem of embedding dissimilarity
measures for directed graphs. In particular, we show in
\S~\ref{sec:embedd-dist-direct} that, in contrast to embeddings through
classical MDS, the embeddings of the distance measures using the
eigensystems of matrices such as the transition matrices or the
Laplacian matrices do not extends to directed graphs. Therefore, the
current presentations in the literature of manifold learning
algorithms such as Laplacian eigenmaps and diffusion maps do not
extend naturally to directed graphs. This reaffirms our view of
decoupling the distances from the embeddings. \\ \\
%
\noindent
Finally, we present in Chapter~\ref{cha:embedd-direct-prox} a novel
approach to embed directed (asymmetric) proximity data. The
approach can be understood as a hybrid three-way MDS and asymmetric MDS
model. Thus, for example, we can embed the mean first passage times
directly, instead of first symmetrizing the mean first
passage times and embed the resulting expected commute times.
%  
%
%
%
