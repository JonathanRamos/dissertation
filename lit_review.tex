...
under the umbrella of \emph{non-linear} dimensionality reduction. We
...
%
%
Manifold learning is then a class of non-Euclidean dimensionality
reduction algorithms that are motivated by the assumption that data
lies in a high-dimensional space but is of low intrinsic
dimension. Examples of data sets where this assumption is reasonable
includes image data
...
explosion of interest in techniques for manifold learning. There are
many reasons for this interest, some of which can be attributed to the
popularity and success of concepts such as graph Laplacians, random
walks on graphs, and spectral decomposition of matrices.  \\ \\
%
%
\noindent The concept of the combinatorial Laplacian of a graph had
been known at least as early as the matrix-tree theorem Kirchoff which
stated that the number of spanning tree of a graph $G$ is equal to any
cofactor of the Laplacian $\mathbf{L}(G)$ of
$G$. \citet{fiedler73:_algeb} then noted that the second smallest
eigenvalue of $\mathbf{L}(G)$ is related to a notion of connectivity
on $G$. This lead to an interest in understanding the spectral
properties of graph Laplacian. See for example
\citet{cvetkovic80:_spect_graph_theor_applic,mohar91:_graph} and the
references within. The normalized Laplacian $\bm{\mathcal{L}}(G)$
appeared mainly through the pioneering work done by Chung and her
...
theory, harmonic analysis, and other diverse disciplines.  Recently,
Laplacians matrices of graphs had appeared in spectral clustering,
e.g., \citet{shi00:_normal}, and manifold learning, e.g.,
\citet{belkin03:_laplac}. Connections between the graph Laplacians
and the Laplace-Beltrami operator on Riemannian manifolds lead to the
study of convergence of discrete Laplacians
\citep{hein05:_from,luxburg08:_consis,belkin08:_towar_theor_found_laplac}.

%The notion of random walks on graphs is also widespread in the
%literature. 

Some of the techniques, such as Isomap, Laplacian eigenmaps
...
system to embed the data points. We will discuss in some details
several of the above mentioned technique in this section.  There are
also additional techniques, some of which is based on probabilistic
models such as maximum-likelihood PCA \citep{tipping99:_mixtur} and
neighbourhoods alignment using GMM \citep{brand05:_chart}, but we will
not be concerned with them in the course of this work. For additional
survey of manifold learning and dimensionality reduction algorithms,
including some that are not discussed here, refer to the survey
articles of \citet{burges05:_data} and \citet{saul06:_semis}. % \\
% \\
% %
% %
% \noindent
...
algorithms. This also ties in with our earlier remark in this section
regarding non-Euclidean dimensionality reduction techniques. \S
\ref{cha:dist-undir-graphs} and \S \ref{cha:dist-direct-graphs} are
devoted to discussing various notions of distances on graphs,
including diffusion distances and expected commute time.
%
%

\section{Spectral methods}
\label{sec:spectral-methods}
