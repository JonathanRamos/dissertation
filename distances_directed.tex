...
\ref{prop:24}, failed to hold for directed
...
\begin{proposition}
  ...
  and aperiodic. Let $f(x)$ be a function with a series
  expansion $f(x) = \sum_{k \geq 0}{a_k x^k}$, $a_k \geq 0\,\, \forall
  k$, with radius of convergence $R \geq 1$. Then there exist a
  $\alpha \in (0,1)$ such that $H(f(\mathbf{P}_\alpha - \mathbf{Q})
  \bm{\Pi}^{-1})$ is a positive semidefinite matrix,
  where $\mathbf{P}_\alpha = (1 - \alpha)\mathbf{I} + \alpha
...
\begin{proof}
  ...
  \mathbf{1}\bm{\pi}^{T}$. To established the claims in Proposition
  \ref{prop:26}, it is sufficient to show that there exist a $\alpha
  \in (0,1)$ such that $\mathbf{P}_\alpha - \mathbf{Q}$ is diagonally
  dominant. Indeed, if $\mathbf{P}_{\alpha} - \mathbf{Q}$ is
  diagonally dominant for some $\alpha \in (0,1)$, then
  $f(\mathbf{P}_{\alpha} - \mathbf{Q})$ is also diagonally dominant
  because the $a_k$ are all non-negative, and thus
  $f(\mathbf{P}_{\alpha} - \mathbf{Q})\bm{\Pi}^{-1}$ is diagonally
  dominant. Furthermore, if $f(\mathbf{P}_{\alpha} - \mathbf{Q})$ is
  diagonally dominant for some $\alpha \in (0,1)$, then
  $f(\mathbf{P}_{\alpha} - \mathbf{Q})$ is also diagonally dominant
  for all $\beta \in (0,\alpha]$. Because $H(f(\mathbf{P}_{\alpha} -
...
$\exp((\mathbf{I} + \mathbf{P})/2)$ is diagonally
