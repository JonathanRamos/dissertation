...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
The series expansion for $H(\mathbf{Z}\bm{\Pi}^{-1})$ is then given by
\begin{equation}
  \label{eq:80}
...
\section{$f(\mathbf{P} - \mathbf{Q})$ and graph
...
\begin{proof}
  ...
  of $f$ is at least $1$. Because 
  \begin{equation}
    \label{eq:126}
    H(f(\mathbf{P}_{\alpha} -
    \mathbf{Q})\bm{\Pi}^{-1}) = \frac{1}{2}(f(\mathbf{P}_{\alpha} -
    \mathbf{Q})\bm{\Pi}^{-1} + f(\hat{\mathbf{P}}_{\alpha} -
    \mathbf{Q})\bm{\Pi}^{-1})   
  \end{equation}
 where $\hat{\mathbf{P}}_{\alpha}$ is the
...
\end{proof} 
Note that Proposition~\ref{prop:26} only indicates that for any
...
are scenarios in which one can find an $\alpha$ that is independent of
$\mathbf{P}$. For example, the matrix $\exp((\mathbf{I} +
\mathbf{P})/2)\bm{\Pi}^{-1}$ has positive semidefinite Hermitian
...
dominant. Similarly, $\exp((\mathbf{I} + \hat{\mathbf{P}})/2)$ is also
diagonally dominant. $H(\exp((\mathbf{I} +
\mathbf{P})/2)\bm{\Pi}^{-1})$ is thus diagonally dominant and hence
positive semidefinite.  $H(\exp((\mathbf{I} +
\mathbf{P})/2)\bm{\Pi}^{-1})$ can be viewed as a diffusion kernel on
directed graphs, extending the  notion of
diffusion kernels for undirected graphs in \citet{kondor02:_diffus}.
%%% Local Variables: 
%%% mode: latex
